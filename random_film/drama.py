...
def new_deaf(url, headers):
    try:
        ...
        url_ = f'https://newdeaf.lol/genre/%D0%B4%D1%80%D0%B0%D0%BC%D0%B0/page/{page}/'
...
def lord(url, headers):
    try:
        ...
        page = choice([i for i in range(1, pagination + 1)])
        url_ = f'https://lo9.lordfilm.lu/filmy/dramy-f/page/{page}/'
        r = requests.get(url=url_, headers=headers)
        soup = BeautifulSoup(r.text, 'lxml')
...
def film_hd(url, headers):
    try:
        ...
        page = choice([i for i in range(1, pagination + 1)])
        url_ = f'https://the.filmhd1080.buzz/drama/page/{page}/'
...
def hd_rezka(url, headers):
    try:
        ...
        page = choice([i for i in range(1, pagination + 1)])
        url_ = f'https://nu10.hdreska.club/genres/50/{page}'
        r = requests.get(url=url_, headers=headers)
        soup = BeautifulSoup(r.text, 'lxml')
        link = choice(soup.find_all('div', class_='th-item')).find('a').get('href')
...
def drama():
    links_arr = ['https://newdeaf.lol/genre/%D0%B4%D1%80%D0%B0%D0%BC%D0%B0/',
                 'https://lo9.lordfilm.lu/filmy/dramy-f/',
                 'https://the.filmhd1080.buzz/drama/',
                 'https://nu10.hdreska.club/genres/50~%D0%94%D1%80%D0%B0%D0%BC%D0%B0']
    ...
    if choice_ == 'https://newdeaf.lol/genre/%D0%B4%D1%80%D0%B0%D0%BC%D0%B0/':
        return new_deaf(url, headers)
    if choice_ == 'https://lo9.lordfilm.lu/filmy/dramy-f/':
        return lord(url, headers)
    if choice_ == 'https://the.filmhd1080.buzz/drama/':
        return film_hd(url, headers)
    if choice_ == 'https://nu10.hdreska.club/genres/50~%D0%94%D1%80%D0%B0%D0%BC%D0%B0':
