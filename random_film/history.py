...
def new_deaf(url, headers):
    try:
        ...
        url_ = f'https://newdeaf.lol/genre/%D0%B8%D1%81%D1%82%D0%BE%D1%80%D0%B8%D1%8F/page/P{page}/'
...
def lord(url, headers):
    try:
        ...
        page = choice([i for i in range(1, pagination + 1)])
        url_ = f'https://lo9.lordfilm.lu/filmy/istoricheskie-f/page/{page}/'
        r = requests.get(url=url_, headers=headers)
        soup = BeautifulSoup(r.text, 'lxml')
...
def film_hd(url, headers):
    try:
        ...
        page = choice([i for i in range(1, pagination + 1)])
        url_ = f'https://the.filmhd1080.buzz/history/page/{page}/'
...
def hd_rezka(url, headers):
    try:
        ...
        page = choice([i for i in range(1, pagination + 1)])
        url_ = f'https://nu10.hdreska.club/genres/1166/{page}'
        r = requests.get(url=url_, headers=headers)
        soup = BeautifulSoup(r.text, 'lxml')
        link = choice(soup.find_all('div', class_='th-item')).find('a').get('href')
...
def history():
    links_arr = ['https://newdeaf.lol/genre/%D0%B8%D1%81%D1%82%D0%BE%D1%80%D0%B8%D1%8F/',
                 'https://lo9.lordfilm.lu/filmy/istoricheskie-f/',
                 'https://the.filmhd1080.buzz/history/',
                 'https://nu10.hdreska.club/genres/1166~%D0%98%D1%81%D1%82%D0%BE%D1%80%D0%B8%D1%8F']
    ...
    if choice_ == 'https://newdeaf.lol/genre/%D0%B8%D1%81%D1%82%D0%BE%D1%80%D0%B8%D1%8F/':
        return new_deaf(url, headers)
    if choice_ == 'https://lo9.lordfilm.lu/filmy/istoricheskie-f/':
        return lord(url, headers)
    if choice_ == 'https://the.filmhd1080.buzz/history/':
        return film_hd(url, headers)
    if choice_ == 'https://nu10.hdreska.club/genres/1166~%D0%98%D1%81%D1%82%D0%BE%D1%80%D0%B8%D1%8F':
