...
def lord_find_need_rate_page(message):
    try:
        ...
        if find_rate:
            url = 'https://lo9.lordfilm.lu/filmy/istoricheskie-f/'
            ...
            while True:
                ...
                url2 = f'https://lo9.lordfilm.lu/filmy/istoricheskie-f/page/{page}/'
...
def lord_random_film_with_need_rate_history(message):
    try:
        page_rate = lord_find_need_rate_page(message)
        if page_rate:
            url = f'https://lo9.lordfilm.lu/filmy/istoricheskie-f/page/{page_rate[0]}/'
            headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) '
                                     'Chrome/107.0.0.0 Safari/537.36'}
