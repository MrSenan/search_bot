...
def lord_find_need_rate_page(message):
    try:
        ...
        if find_rate:
            url = 'https://q.kpfilm.org/f/cat=9/sort=date/order=desc/'
            ...
            while True:
                ...
                url2 = f'https://q.kpfilm.org/f/cat=9/sort=date/order=desc/page/{page}/'
...
def lord_random_film_with_need_rate_action_movie(message):
    try:
        page_rate = lord_find_need_rate_page(message)
        if page_rate:
            url = f'https://q.kpfilm.org/f/cat=9/sort=date/order=desc/page/{page_rate[0]}/'
            headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) '
                                     'Chrome/107.0.0.0 Safari/537.36'}
